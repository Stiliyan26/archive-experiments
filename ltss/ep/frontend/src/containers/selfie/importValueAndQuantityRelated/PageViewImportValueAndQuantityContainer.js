...
class PageViewImportValueAndQuantityContainer extends React.Component {
    componentDidMount() {
    ...
    render() {
        ...
        if (this.props.data) {
            if (this.props.data[Constants.PATH_FOR_LOADING] instanceof Promise) {
                ...
                body = <div className='page-body'>
                    ...
                    <EmbedChangeHistory entity_id={this.props.id} componentPath={`${selfieEntities.ImportValueAndQuantity.view}.changelog`} retrieveType={selfieEntities.ImportValueAndQuantity.pluralCamelCase} />
                </div>;
            }
        }
        ...
        return (
            <div className="page-body-wrapper">
                <NewHeader text={this.props.t(`${selfieEntities.ImportValueAndQuantity.className}._className`)} auth={this.props.auth.userAuthenticated} />

                <EmbedEntityContainer
                    componentPath={`${selfieEntities.ImportValueAndQuantity.view}.${selfieEntities.ImportValueAndQuantity.singleCamelCase}`}
                    loading={this.props.loading}
                    onBeforeChange={(data) => {
                        if (data.id) {
                            history.push(`/${selfieEntities.ImportValueAndQuantity.pluralCamelCase}/` + data.id);
                        } else {
                            history.push(`/${selfieEntities.ImportValueAndQuantity.pluralCamelCase}/add`);
                        }
                    }}
                    retrieve_id={this.props.match.params.importValueAndQuantity_id}
                    entityName={selfieEntities.ImportValueAndQuantity.pluralCamelCase}
                    headerText={this.props.t(`${selfieEntities.ImportValueAndQuantity.className}._className`)}
                    expanded={true}
                    creatable={false}
                    editable={true}
                    columnOverride={
...
function mapStateToProps(state, ownProps) {
    const data = state.rest.importValueAndQuantityView && state.rest.importValueAndQuantityView.importValueAndQuantity && state.rest.importValueAndQuantityView.importValueAndQuantity._embedded ? state.rest.importValueAndQuantityView.importValueAndQuantity._embedded.importValueAndQuantities[0] : undefined;
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageViewImportValueAndQuantityContainer);
