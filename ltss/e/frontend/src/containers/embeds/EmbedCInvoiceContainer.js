...
class EmbedCInvoiceContainer extends React.Component {
	constructor(...args) {
		super(...args);
		this.state = {
	...
	fetchLines(){
		let checkInterval = setInterval(() => {
			let cInvoice = this.props.data;
			if(cInvoice){
				clearInterval(checkInterval)
				this.props.actions.fetchRESTFollow(
					{
						url: `${API_URL}/reports/builder/1?from=CInvoiceRow&select=CInvoiceRow,CInvoiceRow.article&CInvoiceRow.invoice.id=${invoice.id}&page=0&size=5000`,
					},
					...
					response => {
						...
						let lines = data._embedded.hashMaps.map((hashMap) => {
							let line = hashMap.CInvoiceRow;
							line.article = hashMap['CInvoiceRow.article'];
						//	line.transport = hashMap['CInvoiceRow.transport'];
						//	if(line.transport) {
						//		line.transport.tractorUnit = hashMap['CInvoiceRow.transport.tractorUnit'];
						//		line.transport.trailer = hashMap['CInvoiceRow.transport.trailer'];
						//	}
							return line;
						})
						return lines
					},
					'EmbedCInvoiceContainer.fetchLines',
				)
			}
		}, 200)
	}
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(EmbedCInvoiceContainer);
