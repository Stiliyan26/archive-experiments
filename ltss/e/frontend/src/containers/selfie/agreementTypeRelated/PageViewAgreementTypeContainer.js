...
class PageViewAgreementTypeContainer extends React.Component {
    componentDidMount() {
    ...
    render() {
            ...
            if (this.props.data[Constants.PATH_FOR_LOADING] instanceof Promise) {
                ...
                body = <div className='page-body'>
                    ...
                    <EmbedChangeHistory
                        ...
                        componentPath={`${selfieEntities.AgreementType.view}.changelog`}
                        retrieveType={selfieEntities.AgreementType.pluralCamelCase}
                    />
                </div>;
            }
        }
        ...
        return (
            <div className="page-body-wrapper">
                <NewHeader text={this.props.t(`${selfieEntities.AgreementType.className}._className`)} auth={this.props.auth.userAuthenticated} />

                <EmbedEntityContainer
                    componentPath={`${selfieEntities.AgreementType.view}.${selfieEntities.AgreementType.singleCamelCase}`}
                    loading={this.props.loading}
                    onBeforeChange={(data) => {
                        if (data.id) {
                            history.push(`/${selfieEntities.AgreementType.pluralCamelCase}/` + data.id);
                        } else {
                            history.push(`/${selfieEntities.AgreementType.pluralCamelCase}/add`);
                        }
                    }}
                    retrieve_id={this.props.match.params.agreementType_id}
                    entityName={selfieEntities.AgreementType.pluralCamelCase}
                    headerText={this.props.t(`${selfieEntities.AgreementType.className}._className`)}
                    expanded={true}
                    creatable={false}
                    columnOverride={
                        {
...
function mapStateToProps(state, ownProps) {
    const data = state.rest.agreementTypeView && state.rest.agreementTypeView.agreementType && state.rest.agreementTypeView.agreementType._embedded ? state.rest.agreementTypeView.agreementType._embedded.agreementTypes[0] : undefined;
    return {
        auth: state.auth,
        data: data,
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageViewAgreementTypeContainer);
