...
const REPORT_OUT_BALANCE_BY_MONTHS = "/reports/outBalanceByMonths/"
const SLASH = "/"
const REPORT_OUT_BALANCE_BY_MONTHS_2 = "/reports/outBalanceByMonths2/"
const GET = "get"
...
const PIVOT_VALUE_COA_CODE_MESSAGE = "Обща сума за сметката : "
const PIVOT_VALUE_COA_CODE_TOTAL = "ОБЩО:"
const HEADER_COA_NAME = "Сметка"
const ACCESSOR_COA_NAME = "coa_name"
const HEADER_CC_OUT_CODE = "Код организационна единица"
const ACCESSOR_CC_OUT_CODE = "cc_out_code"
const HEADER_CC_OUT_NAME = "Организационна единица"
const ACCESSOR_CC_OUT_NAME = "cc_out_name"
const HEADER_F_OUT_DT_AMOUNT = "Начално салдо ДЕБИТ"
const ACCESSOR_F_OUT_DT_AMOUNT = "f_out_dt_amount"
const HEADER_F_OUT_CT_AMOUNT = "Начално салдо КРЕДИТ"
const ACCESSOR_F_OUT_CT_AMOUNT = "f_out_ct_amount"
const HEADER_O_OUT_DT_AMOUNT = "Оборот ДЕБИТ"
const ACCESSOR_O_OUT_DT_AMOUNT = "o_out_dt_amount"
const HEADER_O_OUT_CT_AMOUNT = "Оборот КРЕДИТ"
const ACCESSOR_O_OUT_CT_AMOUNT = "o_out_ct_amount"
const HEADER_S_OUT_DT_AMOUNT = "Крайно салдо ДЕБИТ"
const ACCESSOR_S_OUT_DT_AMOUNT = "s_out_dt_amount"
const HEADER_S_OUT_CT_AMOUNT = "Крайно салдо КРЕДИТ"
const ACCESSOR_S_OUT_CT_AMOUNT = "s_out_ct_amount"
const CONSOLIDATE_MESSAGE_FIRST = "ОБОРОТНА ВЕДОМОСТ ПО ОРГАНИЗАЦИОННИ ЕДИНИЦИ"
const CONSOLIDATE_MESSAGE_SECOND = "КОНСОЛИДИРАНА ОБОРОТНА ВЕДОМОСТ ПО ОРГАНИЗАЦИОННИ ЕДИНИЦИ"
...
const HEADER_TEXT = "Оборотна ведомост по ПРЦ"
...
const LIST_TYPE_CHART_ACCOUNT = "fChartAccounts"
const LIST_ATTR_CHART_ACCOUNT = "name"
const C_CC_ORGANIZATION_UNIT_LABEL = "Орг. Единица"
const REPORTS_DOT_C_CC_ORGANIZATION_UNIT = "reports.paramCcOrganizationUnit"
const C_CC_ORGANIZATION_UNIT = "CcOrganizationUnit"
const PARAM_CC_ORGANIZATION_UNIT = ".paramCcOrganizationUnit"
const LIST_TYPE_CC_ORGANIZATION_UNIT = "cCcOrganizationUnits"
const LIST_ATTR_CC_ORGANIZATION_UNIT = "name"
const COL_SM_3 = "col-sm-3"
const REFERENCE_FIRST = "Справка по сметка"
const REFERENCE_SECOND = "Справка по ПРЦ"
const COL_MD_6_MX_AUTO_FONT_WEIGHT_BOLD = "col-md-6 mx-auto font-weight-bold"
...
const COMPONENT_PATH_REPORTS_OUT_BALANCE_BY_MONTHS = "reports.outBalanceByMonths"

//Page: can be used as a landing page
//Container: redux container class
class PageReportOutBalanceByMonthsContainer extends React.Component {
	...
	retrieveData(pFromDate, pToDate, pICoaId, pICcoutId, pYear) {
		let outCode = OUT_CODE_01;
		let pSConsolidated = CONSOLIDATED_N;
		if(pICoaId && pFromDate && pToDate && pICcoutId && pYear) {
			import("moment").then(moment => { //TODO we get an error, when we replace the literal("moment") with a constant
				...
				let promise = axios({
					method: GET,
					url: API_URL+REPORT_OUT_BALANCE_BY_MONTHS + outCode + SLASH + fromDate + SLASH + toDate + SLASH + pICoaId + SLASH + pSConsolidated + SLASH + pICcoutId,
	...
	retrieveDataSec(pFromDate, pToDate, pICoaId, pICcoutId, pYear) {
		let outCode = OUT_CODE_01;
		let pSConsolidated = CONSOLIDATED_N;
		if(pICoaId && pFromDate && pToDate && pICcoutId && pYear) {
			import("moment").then(moment => { //TODO we get an error, when we replace the literal("moment") with a constant
				...
				let promise = axios({
					method: GET,
					url: API_URL+REPORT_OUT_BALANCE_BY_MONTHS_2 + outCode + SLASH + fromDate + SLASH + toDate + SLASH + pICoaId + SLASH + pSConsolidated + SLASH + pICcoutId,
					headers: {Authorization: sessionStorage[X_AUTH_TOKEN]}
				}).then(response => {
					let data = response.data;
					console.log(data);
	...
	render() {
		let columns = [
			...
			{
				Header: HEADER_CC_OUT_CODE,
				accessor: ACCESSOR_CC_OUT_CODE,
				aggregate: vals => EMPTY,
			},
			{
				Header: HEADER_CC_OUT_NAME,
				accessor: ACCESSOR_CC_OUT_NAME,
				aggregate: vals => EMPTY,
			},
			{
				Header: HEADER_F_OUT_DT_AMOUNT,
				accessor: ACCESSOR_F_OUT_DT_AMOUNT,
				aggregate: vals => lodash.round(lodash.sum(vals),3),
				Aggregated: row => {return <span><b><i>{row.value}</i></b></span>;},
				Footer: props => {
					let vals = props.data.map((rowData) => (rowData.f_out_dt_amount));
					return <span><b><i>{lodash.round(lodash.sum(vals),3)}</i></b></span>;
				},
			},
			{
				Header: HEADER_F_OUT_CT_AMOUNT,
				accessor: ACCESSOR_F_OUT_CT_AMOUNT,
				aggregate: vals => lodash.round(lodash.sum(vals),3),
				Aggregated: row => {return <span><b><i>{row.value}</i></b></span>;},
				Footer: props => {
					let vals = props.data.map((rowData) => (rowData.f_out_ct_amount));
					return <span><b><i>{lodash.round(lodash.sum(vals),3)}</i></b></span>;
				},
			},
			{
				Header: HEADER_O_OUT_DT_AMOUNT,
				accessor: ACCESSOR_O_OUT_DT_AMOUNT,
				aggregate: vals => lodash.round(lodash.sum(vals),3),
				Aggregated: row => {return <span><b><i>{row.value}</i></b></span>;},
				Footer: props => {
					let vals = props.data.map((rowData) => (rowData.o_out_dt_amount));
					return <span><b><i>{lodash.round(lodash.sum(vals),3)}</i></b></span>;
				},
			},
			{
				Header: HEADER_O_OUT_CT_AMOUNT,
				accessor: ACCESSOR_O_OUT_CT_AMOUNT,
				aggregate: vals => lodash.round(lodash.sum(vals),3),
				Aggregated: row => {return <span><b><i>{row.value}</i></b></span>;},
				Footer: props => {
					let vals = props.data.map((rowData) => (rowData.o_out_ct_amount));
					return <span><b><i>{lodash.round(lodash.sum(vals),3)}</i></b></span>;
				},
			},
			{
				Header: HEADER_S_OUT_DT_AMOUNT,
				accessor: ACCESSOR_S_OUT_DT_AMOUNT,
				aggregate: vals => lodash.round(lodash.sum(vals),3),
				Aggregated: row => {return <span><b><i>{row.value}</i></b></span>;},
				Footer: props => {
					let vals = props.data.map((rowData) => (rowData.s_out_dt_amount));
					return <span><b><i>{lodash.round(lodash.sum(vals),3)}</i></b></span>;
				},
			},
			{
				Header: HEADER_S_OUT_CT_AMOUNT,
				accessor: ACCESSOR_S_OUT_CT_AMOUNT,
				aggregate: vals => lodash.round(lodash.sum(vals),3),
				Aggregated: row => {return <span><b><i>{row.value}</i></b></span>;},
				Footer: props => {
					let vals = props.data.map((rowData) => (rowData.s_out_ct_amount));
					return <span><b><i>{lodash.round(lodash.sum(vals),3)}</i></b></span>;
				},
			},
		];
		...
		return (
			<div className={REPORTS_WRAPPER}>
				...
				<div className={PAGE_BODY}>
					<div className={COL_SM_12_FORM_GROUP}>
						<Form>
							<Form.Row>
								...
								<Col>
									<Form.Group controlId={REPORTS_DOT_PARAM_CHART_ACCOUNT}>
										...
										<FieldSelectContainer
											...
											listType={LIST_TYPE_CHART_ACCOUNT}
											listAttr={LIST_ATTR_CHART_ACCOUNT}
											editable={true}
											creatable={false}
										/>
									</Form.Group>
								...
								<Col>
									<Form.Group controlId={REPORTS_DOT_C_CC_ORGANIZATION_UNIT}>
										<Form.Label>{this.props.t(C_CC_ORGANIZATION_UNIT_LABEL)}: </Form.Label>
										<FieldSelectContainer
											id={C_CC_ORGANIZATION_UNIT}
											componentPath={this.props.componentPath+PARAM_CC_ORGANIZATION_UNIT} //existing path in redux store where we put data
											listType={LIST_TYPE_CC_ORGANIZATION_UNIT}
											listAttr={LIST_ATTR_CC_ORGANIZATION_UNIT}
											editable={true}
											creatable={false}
										/>
									</Form.Group>
							...
							<Form.Row>
								<div className={COL_SM_3}>
									<EmbedRestCallButton componentPath={this.props.componentPath+DOT_RESULT} onClick={(e) => {
										this.retrieveData(this.state.fromDate, this.state.toDate, this.props.data && this.props.data.paramChartAccount ? this.props.data.paramChartAccount.id : undefined, this.props.data && this.props.data.paramCcOrganizationUnit ? this.props.data.paramCcOrganizationUnit.id : undefined, this.state.pYear);
									}}>{this.props.t(REFERENCE_FIRST)}</EmbedRestCallButton>
								</div>
								<div className={COL_SM_3}>
									<EmbedRestCallButton componentPath={this.props.componentPath+DOT_RESULT} onClick={(e) => {
										this.retrieveDataSec(this.state.fromDate, this.state.toDate, this.props.data && this.props.data.paramChartAccount ? this.props.data.paramChartAccount.id : undefined, this.props.data && this.props.data.paramCcOrganizationUnit ? this.props.data.paramCcOrganizationUnit.id : undefined, this.state.pYear);
									}}>{this.props.t(REFERENCE_SECOND)}</EmbedRestCallButton>
								</div>
							</Form.Row>
						</Form>
					</div>
					<div className={COL_MD_6_MX_AUTO_FONT_WEIGHT_BOLD}>
					...
					<div className={COL_SM_12}>
						{reportData && reportData[0] ? reportData[0].param_ccout : EMPTY}
					</div>
					<div className={COL_SM_12}>
						<ReactTable
...
function mapStateToProps(state,ownProps) {
	let componentPath = COMPONENT_PATH_REPORTS_OUT_BALANCE_BY_MONTHS;
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageReportOutBalanceByMonthsContainer);
