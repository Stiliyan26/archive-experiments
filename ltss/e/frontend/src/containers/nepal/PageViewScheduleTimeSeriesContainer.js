...
const DOT = "."
const SCHEDULE_TIME_SERIES = "scheduleTimeSeries"
const SCHEDULE_TIME_SERIES_DOT_CLASS_NAME = "ScheduleTimeSeries._className"
const SCHEDULE_TIME_SERIES_VIEW = "scheduleTimeSeriesView"

//Page: can be used as a landing page
//View: presents details of object
//Container: redux container class
class PageViewScheduleTimeSeriesContainer extends React.Component {	
	componentDidMount(){
	...
	render() {
		...
		if(this.props.data) {
			if(this.props.data[Constants.PATH_FOR_LOADING] instanceof Promise) {
				...
				const intervals = getEntityDefinition("intervals",{scheduleTimeSeries: {show: false}});
				body = <div className={PAGE_BODY}>
						<EmbedRetrieveEntityListContainer
							title={intervals.label}
							icon={intervals.icon}
							columns={intervals.columns}
							componentPath={this.props.componentPath+".intervals"}
							retrieveType="intervals"
							parentHref={this.props.href}
							parentAttr="scheduleTimeSeries"
		...
		return (
			<div className={PAGE_BODY_WRAPPER}>
				...
				<EmbedEntityContainer
					...
					entityName={SCHEDULE_TIME_SERIES}
					headerText={this.props.t(SCHEDULE_TIME_SERIES_DOT_CLASS_NAME)}
					creatable={false}
					expanded={true}
				/>;
				{body}
...
function mapStateToProps(state,ownProps) {
	const entityName = SCHEDULE_TIME_SERIES;
	const entityDef = getEntityDefinition(entityName,undefined);
	const componentPath = SCHEDULE_TIME_SERIES_VIEW;
	...
	return {
		...
		//UI
		headerText: ownProps.t(SCHEDULE_TIME_SERIES_DOT_CLASS_NAME),
	};
}
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageViewScheduleTimeSeriesContainer);
