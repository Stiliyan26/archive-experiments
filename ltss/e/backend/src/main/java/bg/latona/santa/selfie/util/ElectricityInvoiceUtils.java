...
//    }
    ...
    public static LoiDocumentType parseLoiDocumentTypeFromPopulateRequest(Long code, Map<Integer, LoiDocumentType> loiDocumentType) {
        String documentTypeName = code == 1L ? "invoice" : (code == 2L ? "debitNote" : "creditNote");

        if (LOI_DOCUMENT_TYPES_EN_NAMES_MAP.containsKey(documentTypeName))
            return loiDocumentType.get(LOI_DOCUMENT_TYPES_EN_NAMES_MAP.get(documentTypeName).intValue());
