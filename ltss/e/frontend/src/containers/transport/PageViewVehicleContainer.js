...
class PageViewVehicleContainer extends React.Component {	
	componentDidMount(){
	...
	render() {
		...
		if(this.props.data) {
			if(this.props.data[Constants.PATH_FOR_LOADING] instanceof Promise) {
				...
				const vehicleFuelReportsDef = getEntityDefinition("vehicleFuelReports",{vehicle: {show: false}});
				const attachmentDef = getEntityDefinition("assetAttachments",{asset: {show: false}});
				const commentsDef = getEntityDefinition("assetComments",{asset: {show: false}});
				body = <div className='page-body'>
						<EmbedRetrieveEntityListContainer
							title={vehicleFuelReportsDef.label}
							icon={vehicleFuelReportsDef.icon}
							columns={vehicleFuelReportsDef.columns}
							componentPath={this.props.componentPath+".vehicleFuelReports"}
							retrieveType="vehicleFuelReports"
		...
		return (
			<div className="page-body-wrapper">
				...
				<EmbedEntityContainer 
					...
					entityName={"vehicles"}
					headerText={this.props.t("Vehicle._className")}
					creatable={true}
					expanded={true}
				/>
				{body}
...
function mapStateToProps(state,ownProps) {
	...
	const componentPath = "vehicleView";
	...
	return {
		...
		//UI
		headerText: ownProps.t("Vehicle._className"),
	};
}
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageViewVehicleContainer);
