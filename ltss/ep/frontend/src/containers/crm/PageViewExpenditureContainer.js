...
class PageViewExpenditureContainer extends React.Component {	
	componentDidMount(){
	...
	render() {
		...
		if(this.props.data) {
			if(this.props.data[Constants.PATH_FOR_LOADING] instanceof Promise) {
				...
				const taskAttachmentDef = getEntityDefinition("taskAttachments",{
					attachment: {show: false},
					task: {Header: this.props.t("Transport._className"), entityType: "transports"},
				});
				body = <div>
						...
						/>
						<EmbedRetrieveEntityListContainer
							title={this.props.t("Expenditure.attachedToAssets")}
						...
						/>
						<EmbedRetrieveEntityListContainer
							title={this.props.t("Expenditure.attachedToEmployees")}
						...
						/>
						<EmbedRetrieveEntityListContainer
							title={this.props.t("Expenditure.attachedToTasks")}
							...
							parentData={this.props.data}
							defaultFilter={[{id: "task.dtype", value: "Transport"}]}
						/>
...
function mapStateToProps(state,ownProps) {
	const entityName = "expenditures";
	const entityDef = getEntityDefinition(entityName,undefined);
	const componentPath = "expenditureView";
	...
	return {
		...
		//UI
		headerText: ownProps.t("Expenditure._className"),
	};
}
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageViewExpenditureContainer);
