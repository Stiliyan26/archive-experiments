...
class PageViewImportValueContainer extends React.Component {
    componentDidMount() {
    ...
    render() {
        ...
        if (this.props.data) {
            if (this.props.data[Constants.PATH_FOR_LOADING] instanceof Promise) {
                ...
                body = <div className='page-body'>
                    ...
                    <EmbedChangeHistory entity_id={this.props.id} componentPath={`${selfieEntities.ImportValue.view}.changelog`} retrieveType={selfieEntities.ImportValue.pluralCamelCase} />
                </div>;
            }
        }
        ...
        return (
            <div className="page-body-wrapper">
                <NewHeader text={this.props.t(`${selfieEntities.ImportValue.className}._className`)} auth={this.props.auth.userAuthenticated} />

                <EmbedEntityContainer
                    componentPath={`${selfieEntities.ImportValue.view}.${selfieEntities.ImportValue.singleCamelCase}`}
                    loading={this.props.loading}
                    onBeforeChange={(data) => {
                        if (data.id) {
                            history.push(`/${selfieEntities.ImportValue.pluralCamelCase}/` + data.id);
                        } else {
                            history.push(`/${selfieEntities.ImportValue.pluralCamelCase}/add`);
                        }
                    }}
                    retrieve_id={this.props.match.params.importValue_id}
                    entityName={selfieEntities.ImportValue.pluralCamelCase}
                    headerText={this.props.t(`${selfieEntities.ImportValue.className}._className`)}
                    expanded={true}
                    creatable={false}
                    columnOverride={
                        {
...
function mapStateToProps(state, ownProps) {
    const data = state.rest.importValueView && state.rest.importValueView.importValue && state.rest.importValueView.importValue._embedded ? state.rest.importValueView.importValue._embedded.importValues[0] : undefined;
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageViewImportValueContainer);
