...
const SLASH = "/"
const SLASH_ADD = "/add"
const C_CT_BANK_ACCOUNTS = "cCtBankAccounts"
const PAGE_VIEW_C_CT_BANK_ACCOUNT_CONTAINER = "PageViewCCtBankAccountContainer"
const C_CT_BANK_ACCOUNT_CLASS_NAME = "CCtBankAccount._className"

//Page: can be used as a landing page
//View: presents details of object
//Container: redux container class
class PageViewCCtBankAccountContainer extends React.Component {
	componentDidMount(){
		...
		return (
			<div className={PAGE_BODY_WRAPPER}>
				...
				<EmbedEntityContainer
					...
					creatable={false}
					expanded={true}
					columnOverride={{outCode: {show: false},code: {show: false}}}
				/>;
				{body}
			</div>
			);
...
function mapStateToProps(state,ownProps) {
	const entityName = C_CT_BANK_ACCOUNTS;
	const entityDef = getEntityDefinition(entityName,undefined);
	const componentPath = PAGE_VIEW_C_CT_BANK_ACCOUNT_CONTAINER;
	...
	return {
		...
		//UI
		headerText: ownProps.t(C_CT_BANK_ACCOUNT_CLASS_NAME),
	};
}
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageViewCCtBankAccountContainer);
