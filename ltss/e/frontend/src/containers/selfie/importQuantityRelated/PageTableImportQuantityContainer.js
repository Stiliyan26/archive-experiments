...
import { REPORTS_IMPORT_QUANTITIES_FILE, LOGIN, IMPORT_QUANTITIES_FETCH, LEGAL_PEOPLE_FETCH, ACCOUNTING_PERIOD_FETCH } from "../constants/selfiePaths.js";
...
class PageTableImportQuantityContainer extends React.Component {
    componentDidMount() {
        if (!isAuthenticated(this.props.auth)) {
            history.push(LOGIN);
        }

        this.fetchDisplayDataImportQuantities();
    }

    fetchDisplayDataImportQuantities() {
        axios({
            method: "GET",
            url: API_URL + IMPORT_QUANTITIES_FETCH,
            ...
            .then(res => {
                ...
                if (res.data.hasOwnProperty("_embedded")) { // case where there are entries
                    const importQuantitiesData = res.data._embedded.hashMaps;
                    ...
                    importQuantitiesData
                        .forEach(importQuantity => {
                            // add the access point
                            accessPointsSet.add(importQuantity["ImportQuantity"].reportingPointOwn);

                            // increment the Document Type
                            if (importQuantity["ImportQuantity.loiDocumentType"].listOptionItemName === "Фактура") {
                                this.setState((prevState) => ({
                                    ...prevState,
                                    numberOfInvoices: prevState.numberOfInvoices + 1
                                }));

                            } else if (importQuantity["ImportQuantity.loiDocumentType"].listOptionItemName === "Дебитно известие") {
                                this.setState((prevState) => ({
                                    ...prevState,
                    ...
                    axios({
                        ...
                        .then(res1 => {
                            ...
                            this.setState((prevState) => ({
                                ...prevState,
                                numberOfImportQuantitiesInAccountingPeriod: 0
                            }));

                            importQuantitiesData
                                .forEach(importQuantity => {
                                    const periodFromMonth = importQuantity["ImportQuantity"].periodFrom.split("-")[1];
                                    const periodToMonth = importQuantity["ImportQuantity"].periodTo.split("-")[1];
                                    ...
                                    if (
                                        ...
                                        this.setState((prevState) => ({
                                            ...prevState,
                                            numberOfImportQuantitiesInAccountingPeriod: prevState.numberOfImportQuantitiesInAccountingPeriod + 1
                                        }));
                                    }
                                });
                        ...
                        .catch(error => {
                            console.log("PageTableImportQuantityContainer.axios.error" + error);
                        });
                }
            })
            .catch(error => {
    ...
    importExcelImportQuantities(id) {
        console.log("Message from import Excel File: " + id);
        console.log("URL: " + API_URL + REPORTS_IMPORT_QUANTITIES_FILE + id);

        axios({
            method: "POST",
            url: API_URL + REPORTS_IMPORT_QUANTITIES_FILE + id,
            headers: { Authorization: sessionStorage[X_AUTH_TOKEN] }
        })
            .then(response => {
                let data = response.data;

                console.log("PageTableImportQuantityContainer.axios.then", data);
                console.log("PageTableImportQuantityContainer.axios.then -> Object.keys(data).length: ", Object.keys(data).length);
                console.log("PageTableImportQuantityContainer.axios.then -> Object.keys(data): ", Object.keys(data));
                ...
                console.log("Errors: ", this.state.errorArray);
    ...
    render() {
        ...
        // console.log("ALERT: ", this.state.alertUploadHasErrors)
                ...
                <div className='page-body'>
                    <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gridGap: "5px", marginTop: "1.5%", marginBottom: "1%" }}>
                        ...
                        <Card style={{ minHeight: "400px" }} className="custom-card h-100 text-center">
                            <Card.Header style={{ color: 'red' }}>{this.props.t("Selfie.ImportQuantitiesStats")}</Card.Header>
                            ...
                            <Card.Body style={{ color: 'black' }}>
                                ...
                                <ul className="text-start">
                                    ...
                                    <li style={{ textAlign: "left", marginBottom: "0.6rem" }}>{this.props.t("Selfie.NumberOfImportQuantitiesInAccountingPeriod")}: {this.state.numberOfImportQuantitiesInAccountingPeriod}</li>
                                </ul>
                            </Card.Body>

                            <Card.Footer>
                                <NepalDBFileContainer
                                    onFileUpload={(id) => this.importExcelImportQuantities(id)}
                                    // buttonText={this.props.t("NepalDBFile.manufacturer")}
                                    buttonText={this.props.t("Import.ImportFile")}
                                    nepalDbFileInputTagId="fileImportQuantity"
                                />
                            </Card.Footer>
                        </Card>
                    </div>
                    ...
                    {/* 
                        <div class="button" onClick={() => { console.log("Download Errors") }}>
                            <span>
                                Download Excel
                            </span>
                        </div> 
                    */}
                    ...
                    <EmbedRetrieveEntityListContainer
                        ...
                        columns={columns}
                        title={this.props.t(`${selfieEntities.ImportQuantity.className}._className_plural`)}
                        icon={this.props.icon}
                        expanded={true}
                        asTable={true}
                        editable={true}
                        selectedRowsColumns={this.props.selectedRowsColumns}
                        onAfterRetrieve={() => { this.fetchDisplayDataImportQuantities(); }}
                        onCommitChange={() => { this.fetchDisplayDataImportQuantities(); }}
                    >
...
function mapStateToProps(state, ownProps) {
    const entityDef = getEntityDefinition(`${selfieEntities.ImportQuantity.pluralCamelCase}`, undefined);
    const componentPath = "PageTable" + entityDef.className;
    const title = ownProps.t(`${selfieEntities.ImportQuantity.className}._className_plural`);
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageTableImportQuantityContainer);
