...
const SLASH = "/"
const SLASH_ADD = "/add"
const F_CT_REPRESENTATIVES = "fCtRepresentatives"
const PAGE_VIEW_CT_REPRESENTATIVE_CONTAINER = "PageViewCtRepresentativeContainer"
const F_CT_REPRESENTATIVE_CLASS_NAME = "FCtRepresentative._className"

//Page: can be used as a landing page
//View: presents details of object
//Container: redux container class
class PageViewCtRepresentativeContainer extends React.Component {
  componentDidMount(){
    ...
    return (
      <div className={PAGE_BODY_WRAPPER}>
        ...
        <EmbedEntityContainer
          ...
          columnOverride={{
            parId: {show: false}, 
            type: {optionFilter: ((elem) => (elem.listOptionItemCode != 4))},
          }}
        >
          {button}
        </EmbedEntityContainer>
...
function mapStateToProps(state,ownProps) {
  const entityName = F_CT_REPRESENTATIVES;
  const entityDef = getEntityDefinition(entityName, undefined);
  const componentPath = PAGE_VIEW_CT_REPRESENTATIVE_CONTAINER;
  ...
  return {
    ...
    //UI
    headerText: ownProps.t(F_CT_REPRESENTATIVE_CLASS_NAME),
  };
}
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageViewCtRepresentativeContainer);
