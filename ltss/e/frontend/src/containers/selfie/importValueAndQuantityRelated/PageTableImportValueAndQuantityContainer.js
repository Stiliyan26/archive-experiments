...
import { REPORTS_IMPORT_VALUES_AND_QUANTITIES_FILE, LOGIN, IMPORT_VALUES_AND_QUANTITIES_FETCH, ACCOUNTING_PERIOD_FETCH, LEGAL_PEOPLE_FETCH } from "../constants/selfiePaths.js";
...
class PageTableImportValueAndQuantityContainer extends React.Component {
    ...
    componentDidMount() {
        ...
        this.fetchDisplayDataImportValuesAndQuantities();
    }

    fetchDisplayDataImportValuesAndQuantities() {
        axios({
            method: "GET",
            url: API_URL + IMPORT_VALUES_AND_QUANTITIES_FETCH,
            ...
            .then(res => {
                ...
                if (res.data.hasOwnProperty("_embedded")) { // case where there are entries
                    const importValuesAndQuantitiesData = res.data._embedded.hashMaps;
                    ...
                    importValuesAndQuantitiesData
                        .forEach(importValueAndQuantity => {
                            // add the access point
                            accessPointsSet.add(importValueAndQuantity["ImportValueAndQuantity"].reportingPointOwn);

                            // increment the Document Type
                            if (importValueAndQuantity["ImportValueAndQuantity.loiDocumentType"].listOptionItemName === "Фактура") {
                                this.setState((prevState) => ({
                                    ...prevState,
                                    numberOfInvoices: prevState.numberOfInvoices + 1
                                }));

                            } else if (importValueAndQuantity["ImportValueAndQuantity.loiDocumentType"].listOptionItemName === "Дебитно известие") {
                                this.setState((prevState) => ({
                                    ...prevState,
                    ...
                    axios({
                        ...
                        .then(res1 => {
                            ...
                            this.setState((prevState) => ({
                                ...prevState,
                                numberOfImportValuesAndQuantitiesInAccountingPeriod: 0
                            }));

                            importValuesAndQuantitiesData
                                .forEach(importValueAndQuantity => {
                                    const periodFromMonth = importValueAndQuantity["ImportValueAndQuantity"].periodFrom.split("-")[1];
                                    const periodToMonth = importValueAndQuantity["ImportValueAndQuantity"].periodTo.split("-")[1];
                                    ...
                                    if (
                                        ...
                                        this.setState((prevState) => ({
                                            ...prevState,
                                            numberOfImportValuesAndQuantitiesInAccountingPeriod: prevState.numberOfImportValuesAndQuantitiesInAccountingPeriod + 1
                                        }));
                                    }
                                });
                        ...
                        .catch(error => {
                            console.log("PageTableImportValueAndQuantityContainer.axios.error" + error);
                        });
                }
            })
            .catch(error => {
    ...
    importExcelImportValuesAndQuantities(id) {
        console.log("Message from import Excel File: " + id);
        console.log("URL: " + API_URL + REPORTS_IMPORT_VALUES_AND_QUANTITIES_FILE + id);

        axios({
            method: "POST",
            url: API_URL + REPORTS_IMPORT_VALUES_AND_QUANTITIES_FILE + id,
            headers: { Authorization: sessionStorage[X_AUTH_TOKEN] }
        })
            .then(response => {
                let data = response.data;
                console.log("PageTableImportValueAndQuantityContainer.axios.then", data);
                ...
                <div className='page-body'>
                    <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gridGap: "5px", marginTop: "1.5%", marginBottom: "1%" }}>
                        ...
                        <Card style={{ minHeight: "400px" }} className="custom-card h-100 text-center">
                            <Card.Header style={{ color: 'red' }}>{this.props.t("Selfie.ImportValuesAndQuantitiesStats")}</Card.Header>
                            ...
                            <Card.Body style={{ color: 'black' }}>
                                ...
                                <ul className="text-start">
                                    ...
                                    <li style={{ textAlign: "left", marginBottom: "0.6rem" }}>{this.props.t("Selfie.NumberOfImportValuesAndQuantitiesInAccountingPeriod")}: {this.state.numberOfImportValuesAndQuantitiesInAccountingPeriod}</li>
                                </ul>
                            </Card.Body>

                            <Card.Footer>
                                <NepalDBFileContainer
                                    onFileUpload={(id) => this.importExcelImportValuesAndQuantities(id)}
                                    buttonText={this.props.t("Import.ImportFile")}
                                    nepalDbFileInputTagId="fileImportValueAndQuantities"
                                />
                            </Card.Footer>
                        </Card>
                    </div>
                    ...
                    <EmbedRetrieveEntityListContainer
                        ...
                        columns={columns}
                        title={this.props.t(`${selfieEntities.ImportValueAndQuantity.className}._className_plural`)}
                        icon={this.props.icon}
                        expanded={true}
                        asTable={true}
                        editable={true}
                        selectedRowsColumns={this.props.selectedRowsColumns}
                        onAfterRetrieve={() => { this.fetchDisplayDataImportValuesAndQuantities(); }}
                        onCommitChange={() => { this.fetchDisplayDataImportValuesAndQuantities(); }}
                    >
...
function mapStateToProps(state, ownProps) {
    const entityDef = getEntityDefinition(selfieEntities.ImportValueAndQuantity.pluralCamelCase, undefined);
    const componentPath = "PageTable" + entityDef.className;
    const title = ownProps.t(`${selfieEntities.ImportValueAndQuantity.className}._className_plural`);

    return {
        auth: state.auth,
        componentPath: componentPath,
        entityName: selfieEntities.ImportValueAndQuantity.pluralCamelCase,
        entityDef: entityDef,
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageTableImportValueAndQuantityContainer);
