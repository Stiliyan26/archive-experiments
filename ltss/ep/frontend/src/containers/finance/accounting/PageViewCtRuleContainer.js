...
const SLASH = "/"
const SLASH_ADD = "/add"
const F_CT_RULES = "fCtRules"
const PAGE_VIEW_CT_RULE_CONTAINER = "PageViewCtRuleContainer"
const F_CT_RULE_CLASS_NAME = "FCtRule._className"

//Page: can be used as a landing page
//View: presents details of object
//Container: redux container class
class PageViewCtRuleContainer extends React.Component {
	componentDidMount(){
	...
	render() {
		...
		if(this.props.data) {
			if(this.props.data[Constants.PATH_FOR_LOADING] instanceof Promise) {
				...
				const fCtBatchTypeRulesDef = getEntityDefinition(F_CT_BATCH_TYPE_RULES, {rueId: {show: false}});
...
function mapStateToProps(state,ownProps) {
	const entityName = F_CT_RULES;
	const entityDef = getEntityDefinition(entityName, undefined);
	const componentPath = PAGE_VIEW_CT_RULE_CONTAINER;
	...
	return {
		...
		//UI
		headerText: ownProps.t(F_CT_RULE_CLASS_NAME),
	};
}
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageViewCtRuleContainer);
