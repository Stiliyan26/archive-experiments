...
const PAGE_VIEW_TRANSFER_BETWEEN_WAREHOUSES_CONTAINER_DOT_DO_TRANSFER_GOODS = "PageViewTransferBetweenWarehousesContainer.doTransferGoods"
const PATCH = "patch"
const STATUS_21 = "21"
...
const SLASH_ADD = "/add"
const TWO = "2"
const C_SALE_DOT_CLASS_NAME_PP = "CSale._className_PP"
const SLASH_TRANSFER_BETWEEN_WAREHOUSES = "/transferBetweenWarehouses"
const TRANSFER_BETWEEN_WAREHOUSES_VIEW = "TransferBetweenWarehousesView"
const M_2 = "m-2"
...
class PageViewTransferBetweenWarehousesContainer extends React.Component {	
	componentDidMount(){
					...
					getNomenclatureByCriteria(C_TYPE_DOCUMENTS, (elem) => (elem.shortName == PP), this.props.rest, this.props.actions.fetchRESTFollow, this.props.actions.dispatchEditRESTData)
						.then((cTypeDocPP) => {
							...
							this.props.actions.postRESTData(
								...
								PAGE_VIEW_TRANSFER_BETWEEN_WAREHOUSES_CONTAINER_DOT_DO_TRANSFER_GOODS,
		...
		return (
			<div className={PAGE_BODY_WRAPPER}>
				...
				<EmbedEntityContainer
					...
					setDefaults={(data) => {
						if(data == undefined || data.typeDoc == undefined || data.typeDoc.listOptionItemCode != 3) {
							return getNomenclatureByCriteria(LOI_TYPE_DOCS, (elem) => (elem.listOptionItemCode == 3 /*PP*/), this.props.rest, this.props.actions.fetchRESTFollow, this.props.actions.dispatchEditRESTData)
								.then((typeDocPP) => {
									data.typeDoc = typeDocPP;
									if(data._links == undefined) data._links = {};
									if(data._links.typeDoc == undefined) data._links.typeDoc = {};
									data._links.typeDoc.href = typeDocPP._links.self.href;
					...
					headerText={this.props.t(C_SALE_DOT_CLASS_NAME_PP)}
					entityOverride={{pageURL: SLASH_TRANSFER_BETWEEN_WAREHOUSES}}
					creatable={false}
					expanded={true}
					columnOverride={{
						parId: {show: false}, 
						...
						saeId: {show: false}, 
						status: {show: false}, tdtId: {show: false}, typeDoc: {isReadOnly: true}, vatto: {show: false}, oldTypeDoc: {show: false}, ofrId: {show: false}, posted: {show: false}, advanceUsed: {show: false}, cost: {show: false}, totalPayed: {show: false}, payed: {show: false}}}
				>
				{(this.props.id && this.props.data
						...
						&& this.props.data.typeDoc && this.props.data.typeDoc.listOptionItemCode == 3 /*PP*/) ?
					<Form><Form.Row>
...
function mapStateToProps(state,ownProps) {
	...
	entityDef.pageURL = SLASH_TRANSFER_BETWEEN_WAREHOUSES;
	const componentPath = TRANSFER_BETWEEN_WAREHOUSES_VIEW;
	...
	return {
		...
		//UI
		headerText: ownProps.t(C_SALE_DOT_CLASS_NAME_PP),
	};
}
...
const ConnectComponent = connect(mapStateToProps, mapDispatchToProps)(PageViewTransferBetweenWarehousesContainer);
